...
class SequencePredictor(Model):
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.l2_loss(
            preds - y,
    ...
    def add_training_op(self, loss):
        ...
        variables = [output[1] for output in gvs]
        grads = [output[0] for output in gvs]
        
        if self.config.clip_gradients :
            grads = tf.clip_by_global_norm(grads, clip_norm=Config.max_grad_norm)[0]
            
        gvs = [(grads[i], variables[i]) for i in range(len(grads))]
